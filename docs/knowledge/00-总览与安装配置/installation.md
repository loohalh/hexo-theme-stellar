...
在 `2.0.0-rc.1` 发布到 npm 之前，不带版本的安装命令仍会得到 1.x；发布后，同一命令安装当前公开版本。锁定部署或复现问题时使用 `npm install hexo-theme-stellar@2.0.0-rc.1` 这类精确版本。公开 v2 Wiki 不应被当作 1.x 配置参考；升级时以实际安装版本、Release、[CHANGELOG](../../../CHANGELOG.md)和迁移说明为准。同一个站点不要同时保留 `themes/stellar` 源码和 npm 主题包。
