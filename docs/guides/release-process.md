...
只有维护者确认公开发布的版本进入本流程；内部开发里程碑不会创建 npm 版本、Git tag 或 GitHub Release。所有公开版本使用同一套 npm、Tag 和 Release 流程。
...
- 工作区无无关改动（仅允许 `package.json`、`package-lock.json`、`CHANGELOG.md` 与安装知识库有未提交变更，已暂存、未暂存与未跟踪文件都会检查；这些文件仍会在写入前交叉校验）
...
4. 读取 `package.json` 的当前版本，在内存中同时准备 `package.json`、`package-lock.json` 与安装知识库的目标版本内容；任一文件无法安全更新时不写入任何文件
...
- `package-lock.json` 根版本与 `package.json` 不一致，或安装知识库找不到当前包版本：写入前终止，不产生部分更新
- 最终态 `npm run release:check` 失败：恢复包版本、锁文件、CHANGELOG 与安装知识库，修复后重新预演
